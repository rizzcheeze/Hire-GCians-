(function () {
  ...
  const PAGE_URLS = {
    auth: "./hire_gcians_auth.html",
    routes: "./hire_gcians_route_index.html",
    studentDashboard: "./hire_gcians_student_dashboard.html",
    employerDashboard: "./hire_gcians_employer_dashboard.html",
    adminOverview: "./hire_gcians_admin.html",
  };
  ...
  function renderAdminListingsPage() {
    ...
    target.innerHTML = allJobs.map((job) => `
      <div class="t-row jobs-grid">
        ...
        <div style="color:rgba(192,221,151,0.78)">${escapeHtml(job.company || "Unknown company")}</div>
  ...
  function renderAdminAiLogsPage() {
    ...
    const matches = allJobs.slice(0, 6).map((job) => {
      const topStudent = students()
        .map((student) => ({ student, score: scoreJob(student, job) }))
  ...
  function renderAdminAnnouncementsPage() {
    ...
    target.innerHTML = [
      ...
      "Frontend folder isolates the presentation build from backend scaffolding work."
  ...
  function renderAdminSettingsPage() {
    ...
    target.innerHTML = `
      ...
      <div class="panel-row">Presentation route set: public, student, employer, and admin tabs complete</div>
    `;
  }
  ...
  function renderAdminAuditLogsPage() {
    ...
    target.innerHTML = [
      ...
      "Employer job posting draft updated",
      "Admin reports tab opened",
      "Resume match preview recalculated"
